...
exports.run = (client, member) => {
    ...
    sqlite.get(`SELECT * FROM users WHERE userId ="${member.user.id}"`).then(select => {
        if (!select) {
            ...
            console.log('[NEW USERS] - [SALIDA]: ' + member.user.username + '#' + member.user.discriminator + ' (' + member.user.id + ')');
            ...
            sqlite.run('INSERT INTO historiales (idusu, tipo, descripcion, fecha, hora) VALUES (?, ?, ?, ?, ?)', [member.user.id, 'log', 'Salio  de: ' + member.guild.name, datestats, timestats]);
            console.log('[HISTORIAL] - [LOG] - [SALIDA] [' + datestats + ' ' + timestats + ']: ' + member.user.username + '#' + member.user.discriminator + ' (' + member.user.id + ')');
        ...
        console.error(e);
        console.log('[ERROR] - Registrado en [GuildMemberRemove]');
